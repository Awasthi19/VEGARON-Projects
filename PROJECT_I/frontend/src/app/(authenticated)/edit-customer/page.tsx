...
function EditCustomerForm() {
  ...
  const handleSubmit = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      const response = await axios.patch(process.env.NEXT_PUBLIC_EDIT_URL!, {
        ...
        citizenshipIssueDate: EditCustomer.citizenshipIssueDate, // new field
        citizenshipIssueDistrict: EditCustomer.citizenshipIssueDistrict, // new field
        ...
        ampereRating: EditCustomer.ampereRating, // new field
        ...
        receivedAmount: EditCustomer.receivedAmount, // renamed (corrected) field
        gender: EditCustomer.gender, // new field
        spouseName: EditCustomer.spouseName, // new field
        fatherName: EditCustomer.fatherName, // new field
        grandFatherName: EditCustomer.grandFatherName, // new field
        nibedanDartaNUmber: EditCustomer.nibedanDartaNUmber, // new field
        dob: EditCustomer.dob, // new field
  ...
  const handleDateChange = (name: any, value: any) => {
    ...
    setEditCustomer((prevEditCustomer: any) => ({
      ...prevEditCustomer, // Spread the previous state to retain other fields
  ...
  const initDatePickers = () => {
    for (let i = 0; i < 5; i++) {
      ...
      try {
        ...
        // Set the value from EditCustomer if available
        if (EditCustomer && EditCustomer[datepicker.name]) {
          datepicker.value = EditCustomer[datepicker.name];
        }
      } catch (error) {
        console.warn(
  ...
  useEffect(() => {
    if (EditCustomer) {
      // Datepicker should only be initialized if EditCustomer is set
  ...
  }, [EditCustomer]); // Re-run the effect when EditCustomer changes
